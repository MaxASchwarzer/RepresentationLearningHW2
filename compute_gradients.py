...
from models_grad_comp import RNN, GRU
# from models_grad_comp import make_model as TRANSFORMER
...
def run_epoch(model, data, is_train=False, lr=1.0):
	...
	for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
		...
		outputs_T = outputs[-1]
		# Get the last time step target
		targets_T = targets[-1]
		# Compute their loss: L_T
		loss_T = loss_fn(outputs_T, targets_T)
		# Compute all the gradients
		loss_T.backward()
		# Get all the gradients
		grads = model.GetHiddenStateGradients()
...
grads_list_normalized = run_epoch(model, train_data, True)
np.save(file = 'gradient_computation_' + args.model, arr = grads_list_normalized)