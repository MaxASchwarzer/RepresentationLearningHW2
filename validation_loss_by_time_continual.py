...
parser.add_argument('--load', type=str, default='',
					help='directory to load the trained model from')
...
flags = [flag.lstrip('--').replace('/', '').replace('\\', '') for flag in sys.argv[1:] if "load" not in flag]
...
model.load_state_dict(torch.load(args.load + "best_params_" + args.model + ".pt", map_location=device))
...
def run_epoch(model, data, is_train=False, lr=1.0):
	...
	losses = torch.zeros((model.seq_len,))
	# net_loss = np.zeros(model.seq_len)
	for step, (x, y) in enumerate(ptb_iterator(data, model.batch_size, model.seq_len)):
		...
		# 	loss_i = loss_fn(outputs[i], targets[i]).item()
		# 	loss_list.append(loss_i)
		# net_loss = net_loss + np.array(loss_list)
...
val_losses = run_epoch(model, valid_data)
np.save(args.load + args.model + "_losses_continual.npy", val_losses)
